...
std::pair<std::vector<long long>, long long> calcAndPrintDuration(long long n,char m)
{
        ...
        std::cout << "Found " << primes.size() << " primes in " << double(duration) / 1000 << " seconds." << std::endl;
...
{
    std::cout << "Total time elapsed: " << double(duration + durationWrite) / 1000 << " seconds." << std::endl;
}
...
long long outputToFile(const std::vector<long long>& primes, long long n)
{
    ...
    auto diffWrite = std::chrono::duration_cast<std::chrono::milliseconds>(endWrite - startWrite);
    long long durationWrite = diffWrite.count();
    std::cout << "Time elapsed writing to file: " << double(durationWrite) / 1000 << " seconds." << std::endl;
